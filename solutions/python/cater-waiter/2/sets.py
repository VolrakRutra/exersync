...
def check_drinks(drink_name, drink_ingredients):
    ...
    """
    drink_type = "Cocktail" if len(set(drink_ingredients).intersection(ALCOHOLS)) > 0 else "Mocktail"
    return f"{drink_name} {drink_type}"


def categorize_dish(dish_name, dish_ingredients: set[str]):
...
def singleton_ingredients(dishes, intersection):
    ...
    uniques = set()
    for dish in dishes:
        dish_uniques = dish.difference(intersection)
        uniques = uniques.union(dish_uniques)
    return uniques
